...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withItems; query != nil {
		if err := uq.loadItems(ctx, query, nodes, nil,
			func(n *User, e *Item) { n.Edges.Items = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (uq *UserQuery) loadItems(ctx context.Context, query *ItemQuery, nodes []*User, init func(*User), assign func(*User, *Item)) error {
	...
	for i := range nodes {
		if nodes[i].item_owner == nil {
			continue
		}
		fk := *nodes[i].item_owner
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "item_owner" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
